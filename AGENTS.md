...
frond 是為 [spine](https://github.com/yurenju/spine) 而做的。要看某個事實在消費端實際
怎麼被用、或是政策那邊為什麼需要它，**先看 `../spine`**——兩個 repo 通常 clone 在同一個
父目錄底下。

「通常」不是「一定」。兩邊都常以 worktree 開發，也在多個環境之間輪替，所以那個路徑可能
根本不存在。**要用之前先確認**（`ls ../spine`）；不在那裡就從 GitHub 讀——但 spine 是
private repo，讀不到就是讀不到，這時候把「需要確認消費端怎麼用」寫進報告，不要憑猜的寫。

讀 spine 只是為了理解需求。**相依不會反過來**：`src/` 的出貨相依是零（ADR-0005），
spine 不會出現在 `package.json` 裡，也不要為了測試從相對路徑 import 它的東西。
...
PR 說明要用 closing keyword（`Closes #<n>`）關掉它做的那張票——阻塞邊看的是 issue 開不開，漏掉會讓後面的票被假性擋住。另外 frond 的正確性有一部分只有畫面看得出來，所以 PR 說明要放渲染結果的截圖——圖在容器裡產，用 [`pr-image`](https://github.com/yurenju/pr-image) 傳上去內嵌，不 commit 進 repo。兩者的做法與限制見 `docs/agents/pull-requests.md`。
