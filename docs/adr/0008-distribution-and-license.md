...
release note 最上面那一行、寫明「舊的重點可能對不上」的那一種。目前這樣的介入只有一
個：ADR-0006 要求的 `stripScriptedContent`（`<script>`、巢狀瀏覽環境、`on*` 屬性）。
...
- **不能只靠版號說明。** `0.x` 在 semver 裡本來就什麼都不保證（見上面那節），但那句話
  ...
  note 裡自己說一次。
- **要連「哪一批 CFI 會受影響」一起講**：受影響的是移除點之後、同一層的兄弟。frond
  其餘的改寫都是保數的（`link.replaceWith(style)` 是 1:1，frond 自己那兩個 `<style>`
  只 append 到 `<head>`），所以這句話目前指得動的範圍就是那一個介入。
...
會紅的測試，而且有一條寫下來的承諾」。哪一天真的要改成保數的做法，那條路會經過改那
支測試——那正是它存在的目的。
