...
**代價要講清楚**：書宣告 generic family 而讀者沒有設定字型時，跨瀏覽器差分不成立，那類書的正確性只剩另外兩層守著——Node 端的解析測試，以及單一瀏覽器內的不變量與 agent 視覺判讀。而實際的書大多用 generic family 宣告，所以這不是邊緣情況。這是實質的覆蓋率損失，不是文字修飾。
...
**還可以互比的是與分頁無關的量**：解析結果、書寫方向、CFI 的 parse／serialize／compare、以及 agent 視覺判讀那層的結構化缺陷欄位（溢出／重疊／書寫方向／空白頁／裁切）。
