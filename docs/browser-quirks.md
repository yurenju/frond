...
**答案：沒有壞。** 三家都排得出直排、翻得動、回得去原位。Firefox 唯一與另外兩家不同的數字是行推進的 −50.3（另外兩家 −46.3 / −46.8），那是 `h1` 行框寬度的差，在這一節沒有造成任何分頁差異；而三家裡真正排錯東西的是 **WebKit**——直排標點沒有換成直排字符（本檔第一條），Firefox 在那一格是對的。
